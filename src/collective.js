...
function evidenceTools(rows = []) {
  ...
  const oneEvidencePerContribution = (ids = []) => {
    const firstByContribution = new Map();

    for (const id of cleanEvidenceIds(ids)) {
      const contributionId = analysisToContribution.get(id);
      if (!contributionId || firstByContribution.has(contributionId)) continue;
      firstByContribution.set(contributionId, id);
    }

    return [...firstByContribution.values()];
  };

  return {
    cleanEvidenceIds,
    distinctContributionCount,
    oneEvidencePerContribution
  };
}
  ...
  for (const observation of raw.single_contribution_observations || []) {
    ...
    if (contributionCount === 1) {
      singleObservations.push({
        ...observation,
        evidence_ids: oneEvidencePerContribution(evidenceIds)
      });
