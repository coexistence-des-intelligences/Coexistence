...
declare
  ...
  v_historical_case_exists boolean := false;
begin

  select exists (
    select 1
    from public.collective_syntheses
    where public_id = 'S-56389CD6'
      and protocol_version = '0.3'
  )
  into v_historical_case_exists;
  ...
  /*
   * Les descriptions actives issues d'une synthèse 0.3 sont retirées
   * de la carte vivante, mais leur texte reste intact dans chaque
   * synthèse historique. La sélection repose sur le protocole et les
   * clés réellement présentes, jamais sur un identifiant propre à une
   * seule instance.
   */
  ...
  if not exists (
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'observed_case', case
          when v_historical_case_exists then 'S-56389CD6'
          else null
        end,
...
/*
 * Le cas S-56389CD6 appartient à l'instance d'origine. Il ne devient
 * une preuve locale que s'il existe réellement dans l'instance qui
 * applique la migration. Une réplique conserve ainsi une provenance
 * vide plutôt qu'un identifiant inexistant.
 */
update public.governance_items
set
  evidence_ids = case
    when coalesce(evidence_ids, '[]'::jsonb) @> '["S-56389CD6"]'::jsonb
      then evidence_ids
    else coalesce(evidence_ids, '[]'::jsonb) || '["S-56389CD6"]'::jsonb
  end,
  updated_at = now()
